...
def prediction_class_index(result):
    return int(numpy.argmax(result[0]))


def stored_prediction_class_index(prediction_vector_str):
    values = numpy.array([float(x) for x in re.findall(r'\d+\.?\d*', prediction_vector_str)])
    return int(numpy.argmax(values))


def symbol_text_from_prediction(result, predictions_list):
    predicted_idx = prediction_class_index(result)
    for prediction in predictions_list:
        if len(prediction) == 2 and stored_prediction_class_index(prediction[1]) == predicted_idx:
            return prediction[0]
    return ''
...
class Symbol:
    ...
    def __init__(self, matrix, rectangle, model, predictions_list):        
        ...
        self.text = symbol_text_from_prediction(result, predictions_list)
